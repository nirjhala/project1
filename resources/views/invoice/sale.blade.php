...
@endphp
<title>{{ $sale->full_invoice_no ?: '' }} | Sale Invoice</title>
<style>
    body {
        margin: 0;
    }
    ...
    <table class="table">
        <tr>
            <td colspan="6" rowspan="3" class="text-center" style="padding: 10px 0; text-align: center;">
                <div>
                    <h1 
                        style="margin: 0; line-height: 100%; margin-bottom: 10px;"
                    >{{ $sale->school->name }}</h1>
                    <div>{{ $sale->school->user->full_address }}</div>
                    <div>Mob. {{ $sale->school->user->mobile ?: '-' }} Phn. {{ $sale->school->phone ?: '-' }}</div>
                    <div>Email: {{ $sale->school->user->email }}</div>
                </div>
            </td>
            <th colspan="2" class="head-top">Invoice No.</th>
            <td colspan="2">{{ $sale->full_invoice_no }}</td>
        </tr>
        <tr>
            <th colspan="2" class="head-top">Payment Mode</th>
            <td colspan="2">{{ $sale->payment_mode }}</td>
        </tr>
        <tr>
            <th colspan="2" class="head-top">Date &amp; Time</th>
            <td colspan="2">{{ date('d/m/Y h:i A', strtotime($sale->created_at)) }}</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td colspan="3">{{ $sale->customer->name }}</td>
            <td colspan="2">Name</td>
            <td colspan="3">{{ $sale->customer->name }}</td>
        </tr>
        <tr>
            <td colspan="2">Mobile No.</td>
            <td colspan="3">{{ $sale->customer->mobile }}</td>
            <td colspan="2">Mobile No.</td>
            <td colspan="3">{{ $sale->customer->mobile }}</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td colspan="2">{{ $sale->customer->cityData->stateName->name }}</td>
            <td>{{ sprintf("%02d", $sale->customer->cityData->stateName->code) }}</td>
            <td colspan="2">State</td>
            <td colspan="2">{{ $sale->customer->cityData->stateName->name }}</td>
            <td>{{ sprintf("%02d", $sale->customer->cityData->stateName->code) }}</td>
        </tr>
        <tr>
            ...
            @if($sale->customer->cityData->state == $sale->school->city->state)
        ...
        @endforeach
        @php
        $total = $sale->total_amount + $totalGST;
        $totalRow = ($sale->customer->cityData->state == $sale->school->city->state) ? 5 : 4;
        @endphp
        <tr>
            <td rowspan="{{ $totalRow }}" colspan="7" style="text-align: center; padding-bottom: 5px;">
                ...
                <div style="margin-bottom: 100px; text-align: left;">For <strong>{{ $sale->school->name }}</strong></div>
        ...
        <!-- <tr>
            <td colspan="3" style="height: 100px; vertical-align: top; padding-bottom: 5px;">
                ...
                <ul style="list-style: disc inside; margin: 0; padding: 0; padding-left: 5px;">
                    <li>Make all checks payable to {{ $sale->customer->organization_name }}</li>
                </ul>
            </td>
            <td colspan="3" style="text-align: center; vertical-align: bottom; padding-bottom: 5px;">
                <div style="margin-bottom: 80px;">For <strong>{{ $sale->customer->organization_name }}</strong></div>
                <div>Seal / Signatory</div>
            </td>
        </tr> -->
    </table>
