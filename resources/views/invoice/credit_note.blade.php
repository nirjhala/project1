...
@endphp
<title>{{ sprintf("#%05d", $credit_note->number) }} | Credit Note</title>
<style>
    body {
        margin: 0;
    }
    ...
    <table class="table">
        <tr>
            <td colspan="6" rowspan="4" class="text-center" style="padding: 10px 0; text-align: center;">
                <div>
                    ...
                    >{{ $credit_note->sale->school->name }}</h1>
                    <div>{{ $credit_note->sale->school->user->full_address }}</div>
                    <div>Mob. {{ $credit_note->sale->school->user->mobile ?: '-' }} Phn. {{ $credit_note->sale->school->phone ?: '-' }}</div>
                    <div>Email: {{ $credit_note->sale->school->user->email }}</div>
                </div>
            </td>
            <th colspan="2" class="head-top">Invoice No.</th>
            <td colspan="2">{{ $credit_note->sale->full_invoice_no }}</td>
        </tr>
        <tr>
            <th colspan="2" class="head-top">Number #</th>
            <td colspan="2">{{ sprintf('%05d', $credit_note->number) }}</td>
        </tr>
        <tr>
            <th colspan="2" class="head-top">Payment Mode</th>
            <td colspan="2">{{ $credit_note->payment_mode }}</td>
        </tr>
        <tr>
            <th colspan="2" class="head-top">Date &amp; Time</th>
            <td colspan="2">{{ date('d/m/Y h:i A', strtotime($credit_note->created_at)) }}</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td colspan="3">{{ $credit_note->sale->customer->name }}</td>
            <td colspan="2">Name</td>
            <td colspan="3">{{ $credit_note->sale->customer->name }}</td>
        </tr>
        <tr>
            <td colspan="2">Mobile No.</td>
            <td colspan="3">{{ $credit_note->sale->customer->mobile }}</td>
            <td colspan="2">Mobile No.</td>
            <td colspan="3">{{ $credit_note->sale->customer->mobile }}</td>
        </tr>
        <tr>
            <td colspan="2">Address</td>
            <td colspan="3">{{ $credit_note->sale->customer->full_address }}</td>
            <td colspan="2">Address</td>
            <td colspan="3">{{ $credit_note->sale->customer->full_address }}</td>
        </tr>
        <tr>
            <td colspan="2">State</td>
            <td colspan="2">{{ $credit_note->sale->customer->cityData->stateName->name }}</td>
            <td>{{ sprintf("%02d", $credit_note->sale->customer->cityData->stateName->code) }}</td>
            <td colspan="2">State</td>
            <td colspan="2">{{ $credit_note->sale->customer->cityData->stateName->name }}</td>
            <td>{{ sprintf("%02d", $credit_note->sale->customer->cityData->stateName->code) }}</td>
        </tr>
        <tr>
            ...
            @if($credit_note->sale->customer->cityData->state == $credit_note->sale->school->city->state)
        ...
        @foreach($credit_note->items as $index => $item)
        @php
        ...
        @endforeach
        @php
        $total = $credit_note->total_amount + $totalGST;
        $totalRow = ($credit_note->sale->customer->cityData->state == $credit_note->sale->school->city->state) ? 5 : 4;
        @endphp
        <tr>
            <td rowspan="{{ $totalRow }}" colspan="7" style="text-align: center; padding-bottom: 5px;">
                ...
                <div style="margin-bottom: 100px; text-align: left;">For <strong>{{ $credit_note->sale->school->name }}</strong></div>
                <div>Seal / Signatory</div>
            </td>
            <th colspan="2">TOTAL AMOUNT</th>
            <td>Rs. {{ number_format($credit_note->total_amount, 2) }}</td>
        </tr>
        @if($credit_note->sale->customer->cityData->state == $credit_note->sale->school->city->state)
        <tr>
        ...
        <!-- <tr>
            <td colspan="3" style="height: 100px; vertical-align: top; padding-bottom: 5px;">
                ...
                <ul style="list-style: disc inside; margin: 0; padding: 0; padding-left: 5px;">
                    <li>Make all checks payable to {{ $credit_note->sale->customer->organization_name }}</li>
                </ul>
            </td>
            <td colspan="3" style="text-align: center; vertical-align: bottom; padding-bottom: 5px;">
                <div style="margin-bottom: 80px;">For <strong>{{ $credit_note->sale->customer->organization_name }}</strong></div>
                <div>Seal / Signatory</div>
            </td>
        </tr> -->
    </table>
