...
@endphp
<title>{{ sprintf("#%05d", $debit_note->number) }} | Debit Note</title>
<style>
    body {
        margin: 0;
    }
    ...
    <table class="table">
        <tr>
            <td colspan="6" rowspan="4" class="text-center" style="padding: 10px 0; text-align: center;">
                <div>
                    <h1 
                        style="margin: 0; line-height: 100%; margin-bottom: 10px;"
                    >{{ $debit_note->purchase->supplier->organization_name }}</h1>
                    <div>{{ $debit_note->purchase->supplier->full_address }}</div>
                    <div>Mob. {{ $debit_note->purchase->supplier->mobile ?: '-' }} Phn. {{ $debit_note->purchase->supplier->phone ?: '-' }}</div>
                    <div>Email: {{ $debit_note->purchase->supplier->email }}</div>
                </div>
            </td>
            <th colspan="2" class="head-top">Invoice No.</th>
            <td colspan="2">{{ $debit_note->purchase->invoice_no }}</td>
        </tr>
        <tr>
            <th colspan="2" class="head-top">Number #</th>
            <td colspan="2">{{ sprintf('%05d', $debit_note->number) }}</td>
        </tr>
        <tr>
            <th colspan="2" class="head-top">Payment Mode</th>
            <td colspan="2">{{ $debit_note->payment_mode }}</td>
        </tr>
        <tr>
            <th colspan="2" class="head-top">Date &amp; Time</th>
            <td colspan="2">{{ date('d/m/Y h:i A', strtotime($debit_note->created_at)) }}</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td colspan="3">{{ $debit_note->purchase->school->user->name }}</td>
            <td colspan="2">Name</td>
            <td colspan="3">{{ $debit_note->purchase->school->user->name }}</td>
        </tr>
        <tr>
            <td colspan="2">School Name</td>
            <td colspan="3">{{ $debit_note->purchase->school->name }}</td>
            <td colspan="2">School Name</td>
            <td colspan="3">{{ $debit_note->purchase->school->name }}</td>
        </tr>
        <tr>
            <td colspan="2">Mobile No.</td>
            <td colspan="3">{{ $debit_note->purchase->school->user->mobile }}</td>
            <td colspan="2">Mobile No.</td>
            <td colspan="3">{{ $debit_note->purchase->school->user->mobile }}</td>
        </tr>
        <tr>
            <td colspan="2">Address</td>
            <td colspan="3">{{ $debit_note->purchase->school->user->full_address }}</td>
            <td colspan="2">Address</td>
            <td colspan="3">{{ $debit_note->purchase->school->user->full_address }}</td>
        </tr>
        <tr>
            <td colspan="2">State</td>
            <td colspan="2">{{ $debit_note->purchase->school->city->stateName->name }}</td>
            <td>{{ sprintf("%02d", $debit_note->purchase->school->city->stateName->code) }}</td>
            <td colspan="2">State</td>
            <td colspan="2">{{ $debit_note->purchase->school->city->stateName->name }}</td>
            <td>{{ sprintf("%02d", $debit_note->purchase->school->city->stateName->code) }}</td>
        </tr>
        <tr>
            ...
            @if($debit_note->purchase->supplier->city->state == $debit_note->purchase->school->city->state)
        ...
        @foreach($debit_note->items as $index => $item)
        @php
        ...
        @endforeach
        @php
        $total = $debit_note->total_amount + $totalGST;
        $totalRow = ($debit_note->purchase->supplier->city->state == $debit_note->purchase->school->city->state) ? 5 : 4;
        @endphp
        <tr>
            <td rowspan="{{ $totalRow }}" colspan="7" style="text-align: center; padding-bottom: 5px;">
                <div style="font-size: 8px; text-transform: capitilize; text-align: left; margin-bottom: 10px;">
                    (<strong>In words:</strong> {{ getIndianCurrency(ceil($total)) }} Only)
                </div>
                <div style="margin-bottom: 100px; text-align: left;">For <strong>{{ $debit_note->purchase->supplier->organization_name }}</strong></div>
                <div>Seal / Signatory</div>
            </td>
            <th colspan="2">TOTAL AMOUNT</th>
            <td>Rs. {{ number_format($debit_note->total_amount, 2) }}</td>
        </tr>
        @if($debit_note->purchase->supplier->city->state == $debit_note->purchase->school->city->state)
        <tr>
        ...
        <!-- <tr>
            <td colspan="3" style="height: 100px; vertical-align: top; padding-bottom: 5px;">
                ...
                <ul style="list-style: disc inside; margin: 0; padding: 0; padding-left: 5px;">
                    <li>Make all checks payable to {{ $debit_note->purchase->supplier->organization_name }}</li>
                </ul>
            </td>
            <td colspan="3" style="text-align: center; vertical-align: bottom; padding-bottom: 5px;">
                <div style="margin-bottom: 80px;">For <strong>{{ $debit_note->purchase->supplier->organization_name }}</strong></div>
                <div>Seal / Signatory</div>
            </td>
        </tr> -->
    </table>
