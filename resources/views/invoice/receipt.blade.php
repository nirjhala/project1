...
@endphp
<title>{{ sprintf('#%05d', $receipt->receipt_no) }} | Receipt</title>
<style>
    body {
        margin: 0;
    }
    ...
    <table class="table">
        <tr>
            ...
            ><strong>Receipt #</strong> <big style="font-size: 2em;">{{ sprintf('%05d', $receipt->receipt_no) }}</big></td>
            <td colspan="8" class="text-center" style="padding: 10px 0; text-align: center;">
                <div>
                    <h1 
                        style="margin: 0; line-height: 100%; margin-bottom: 10px;"
                    >{{ $receipt->sale->school->name }}</h1>
                    <div>{{ $receipt->sale->school->user->full_address }}</div>
                    <div>Mob. {{ $receipt->sale->school->user->mobile ?: '-' }} Phn. {{ $receipt->sale->school->phone ?: '-' }}</div>
                    <div>Email: {{ $receipt->sale->school->user->email }}</div>
                </div>
            </td>
            <td
                style="text-align: right; padding-right: 15px;"
            ><strong>Date:</strong> {{ date('d.m.Y h:i A', strtotime($receipt->created_at)) }}</td>
        </tr>
        <tr>
            <td
                style="padding-left: 15px; padding-top: 30px"
            ><strong>#Ref Invoice No.</strong> {{ $receipt->sale->full_invoice_no }}</td>
            <td colspan="8" style="text-align: center; padding-top: 30px">
                <h2>RECEIPT</h2>
            </td>
            <td
                style="padding-right: 15px; text-align: right; padding-top: 30px"
            ><strong>Payment Mode</strong> {{ $receipt->payment_mode }}</td>
        </tr>
        <tr>
            <td colspan="10">
                <div class="receipt_body">
                    ...
                    <span 
                        ...
                        style="width: 220px;"
                    >{{ $receipt->sale->customer_name }}</span> of Rs. 
                    <span 
                        class="underline_span"
                        style="width: 220px;"
                    >{{ number_format($receipt->amount, 2) }}</span><br> 
                    In Words: 
                    <span 
                        class="underline_span"
                        style="width: 620px;"
                    >{{ getIndianCurrency($receipt->amount) }}</span><br>
                    Remarks: 
                    <span
                        class="underline_span"
                        style="width: 625px;"
                    >{!! $receipt->remarks ?: 'N/A' !!}</span>
                </div>
            </td>
        </tr>
        <tr>
