...
<div class="dashboard-content-one">
    ...
    <div class="breadcrumbs-area">
        <div class="float-right">
            <router-link :to="{ name: 'add-hostel' }" class="fw-btn-fill btn-gradient-yellow text-white">Add New</router-link>
        </div>
        <h3>Hostel</h3>
        <ul>
            <li>
                        ...
                        <table class="table display data-table text-nowrap">
                            ...
                            <tbody>
                                <tr v-for="(list, i) in lists.data" :key="i">
                                    ...
                                    <td><router-link :to="{ path: 'edit-hostel/'+list.id, params: list }" data-toggle="tooltip" title="Edit"><i class="fa fa-edit"></i> {{ list.name }}</router-link></td>
                                    <td>{{ list.total_rooms }}</td>
                                    <td>Rs. {{ list.cost }}</td>
                                    <td>{{ list.capacity }}</td>
                                    <td>{{ list.address }}</td>
                                    <td><span v-if="list.city_name && list.city_name.state_name">{{ list.city_name.state_name.name }}</span></td>
                                    <td><span v-if="list.city_name">{{ list.city_name.name }}</span></td>
                                    <td><span>{{ list.pincode }}</span></td>
                                </tr>
                            </tbody>
                        </table>
...
export default {
    ...
    methods: {
        ...
        searchData(page) {
            ...
            instance.get('search-hostel/?page='+page+'&s='+s)
                .then(res => {
                    this.loaded = 1;
                    if(res.status) {
        ...
        getRecords(page) {
            ...
            instance.get('view-hostel/?page='+page)
                .then(res => {
                    this.loaded = 1;
                    if(res.status) {
                        this.lists  = res.data.data;
            ...
            if( this.check.length > 0 ) {
                ...
                .then((willDelete) => {
                  if (willDelete) {
                      ...
                      instance.post('remove-hostel', params)
                          .then(res => {
                              if(res.status) {
                                  this.getRecords();
