    ...
    <div class="dashboard-content-one">
        ...
        <div class="breadcrumbs-area">
            <div class="float-right">
                <router-link :to="{ name: 'add-subject' }" class="fw-btn-fill btn-gradient-yellow text-white">Add New</router-link>
            </div>
            <h3>Subject</h3>
            <ul>
                <li>
                            ...
                            <table class="table display data-table text-nowrap">
                                ...
                                <tbody>
                                    <tr v-for="(list, i) in lists.data">
                                        ...
                                        <td>
                                            <router-link :to="{ path: 'edit-subject/'+list.id, params: list }" data-toggle="tooltip" title="Edit"><i class="fa fa-edit"></i> {{ list.name }}</router-link>
                                        </td>
                                        <td>
                                            <ol class="m-0 p-0" style="list-style: inside decimal;" v-if="list.classes">
                                                <li v-for="(cls, j) in list.classes">{{ cls.cls.name }} - {{ cls.cls.dept.dept_name }}</li>
                                            </ol>
                                            <button type="button" class="btn btn-primary addClassBtn" v-on:click="showClassPop( list.id, list.classes )">Add Class</button>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <ul class="list-item-group">
                            <li class="list-item" v-for="cls in allClasses">
                                <label>
                                    <input type="checkbox" v-model="classes" :value="cls.id">
                                    {{ cls.name }} ({{ cls.dept.dept_name }})
                                </label>
                            </li>
                        </ul>
                    </div>
                    <div class="modal-footer">
                        <button class="btn btn-primary" v-on:click="assignClass()">Submit</button>
...
export default {
    ...
    methods: {
        ...
        assignClass() {
            ...
            instance.post('assign-subject-class', params)
                 .then(res => {
                     this.loaded = 1;
        ...
        searchData(page) {
            ...
            instance.get('search-subject/?page='+page+'&s='+s)
                .then(res => {
                    this.loaded = 1;
                    if(res.status) {
        ...
        getRecords(page) {
            ...
            instance.get('view-subject/?page='+page)
                .then(res => {
                    this.loaded = 1;
                    if(res.status) {
                        this.lists  = res.data.data;
            ...
            if( this.check.length > 0 ) {
                ...
                .then((willDelete) => {
                  if (willDelete) {
                      ...
                      instance.post('remove-subject', params)
                          .then(res => {
                              if(res.status) {
                                  this.getRecords();
