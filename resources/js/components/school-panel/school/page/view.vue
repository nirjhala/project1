    ...
    <div class="dashboard-content-one">
        ...
        <div class="breadcrumbs-area">
            <div class="float-right">
                <router-link :to="{ name: 'AddPage' }" class="fw-btn-fill btn-gradient-yellow text-white">Add New</router-link>
            </div>
            <h3>Page</h3>
            <ul>
                <li>
                                ...
                                <table class="table display data-table text-nowrap">
                                    ...
                                    <tbody>
                                        <tr v-for="(list, i) in lists.data" v-bind:key="i">
                                            ...
                                            <td>
                                                <router-link :to="{ name: 'EditPage', params: { id: list.id } }" data-toggle="tooltip" title="Edit"><i class="fa fa-edit"></i> {{ list.title }}</router-link>
                                            </td>
                                            <td>{{ list.template_name ? list.template_name : 'DEFAULT' }}</td>
                                            <td>{{ list.excerpt }}</td>
                                        </tr>
                                    </tbody>
                                </table>
...
export default {
    ...
    methods: {
        ...
        getRecords(page) {
            ...
            this.loaded = 0;
            let apiResponse = api.getPages(page, this.s)
        ...
        deleteRecord() {
            ...
            if( this.check.length > 0 ) {
                ...
                .then((willDelete) => {
                  if (willDelete) {
                      this.loaded = 0;
                      let apiResponse = api.deletePages(params)
