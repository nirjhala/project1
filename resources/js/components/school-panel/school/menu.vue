<template>
  <div class="dashboard-content-one">
    ...
    <div v-if="loaded">
      <b-row>
        <b-col sm="4">
          <b-card header="Add Menu">
            <form method="post" @submit.prevent="addMenu">
              <b-form-group label="Select Menu Location">
                <label>
              ...
              <b-form-group label="Select Page" v-if="form.type == 'Page'">
                <select v-model="form.p_id" class="form-control" required>
                  <option value="">Select Page</option>
                  <option :value="id" v-for="(title, id) in pages" :key="id">
                    {{ title }}
                  </option>
                </select>
              </b-form-group>
              <b-form-group label="Enter Link" v-if="form.type != 'Page'">
                <input
                  v-model="form.link"
                  type="text"
                  class="form-control"
              ...
              <button
                type="submit"
                class="btn-fill-lg btn-block btn-gradient-yellow btn-hover-bluedark"
              >
                Save
              </button>
            </form>
          ...
          <b-card no-body>
            <b-tabs card>
              <b-tab :title="m.name" v-for="(m, i) in menu" :key="i">
                <div v-if="!m.menu.length">No menu added yet.</div>
