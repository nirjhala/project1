            ...
            <div class="row">
              <div class="col-lg-9">
                ...
                <div class="row">
                  <div class="col-xl-6 col-12 form-group">
                    ...
                    >
                      ...
                      <option :value="s.id" v-for="(s, i) in sessions" :key="i">
                        {{ s.session_name }}
                      </option>
                    </select>
                  </div>
                ...
                <div class="form-group">
                  ...
                  <editor v-model="form.description" :init="editor_options" />
                </div>
              </div>
              <div class="col-lg-3">
                <div class="mb-3">
                ...
                <div>
                  ...
                  >
                    <div
                      ...
                      v-for="(dt, i) in ev_dates"
                      :key="i"
                    >
                      <input
                        type="date"
                        class="form-control"
                        v-model="ev_dates[i].date"
                        @change="change_ev_dates"
                      />
                      <span
                      ...
                      >
                        <button type="button" class="btn btn-dark">
                          <i class="icon-minus position-static text-white"></i>
                        </button>
                      </span>
                    </div>
                  </div>
...
export default {
  ...
  methods: {
    ...
    change_ev_dates() {
      this.form.dates = [];
      console.log("ev dates", this.ev_dates);
    ...
    getInfo(id) {
      ...
      apiResponse.then((res) => {
        ...
        if (res.data.dates && res.data.dates.length) {
          this.ev_dates = [];
          res.data.dates.forEach((row, i) => {
            this.ev_dates.push({
              date: row,
            });
            // this.ev_dates = res.data.dates;
          });
        }
        console.log(this.ev_dates);
      });
    },
    addData: function () {
      let params = {
      ...
      apiResponse
        ...
        .catch((error) => {
          this.loaded = 1;
          // console.log(error);
          if (error.response) this.$toast.error("Event already exists.");
        });
    },
    updateData: function () {
      let params = {
