    ...
    <div class="dashboard-content-one">
        ...
        <div class="breadcrumbs-area">
            <div class="float-right">
                <router-link :to="{ name: 'AddNotice' }" class="fw-btn-fill btn-gradient-yellow text-white">Add New</router-link>
            </div>
            <h3>Notice</h3>
            <ul>
                <li>
                                ...
                                <table class="table display data-table text-nowrap">
                                    ...
                                    <tbody>
                                        <tr v-for="(list, i) in lists.data" v-bind:key="i">
                                            ...
                                            <td>
                                                <router-link :to="{ name: 'EditNotice', params: { id: list.id } }" data-toggle="tooltip" title="Edit"><i class="fa fa-edit"></i> {{ list.full_title }}</router-link>
                                            </td>
                                            <td>{{ list.updated_at | formatDate }}</td>
                                        </tr>
                                    </tbody>
                                </table>
...
export default {
    ...
    methods: {
        ...
        getRecords(page) {
            ...
            this.loaded = 0;
            let apiResponse = api.getNotice(page, this.s)
        ...
        deleteRecord() {
            ...
            if( this.check.length > 0 ) {
                ...
                .then((willDelete) => {
                  if (willDelete) {
                      this.loaded = 0;
                      let apiResponse = api.deleteNotice(params)
