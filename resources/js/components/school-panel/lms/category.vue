...
<div class="dashboard-content-one">
    ...
    <div v-if="loaded">
        ...
        <div class="row">
            <div class="col-sm-4">
                <form class="new-added-form" id="addCategoryForm" @submit.prevent=" id ? updateData(id) : addData()">
                                      ...
                                      <table class="table display data-table text-nowrap">
                                          ...
                                          <tbody>
                                              <tr v-for="(list, i) in lists.data">
                                                  ...
                                                  <td>
                                                      <router-link :to="{ path: baseURL+'book-category/edit/'+list.id, params: list }" v-on:click="rerenderSelf" data-toggle="tooltip" title="Edit"><i class="fa fa-edit"></i> {{ list.name }}</router-link>
                                                      <div><small>({{ list.classes_count }} books)</small></div>
                                                  </td>
                                              </tr>
                                          </tbody>
                                      </table>
...
export default {
    ...
    methods: {
        ...
        getInfo(id) {
            ...
            instance.get('book/category/info/'+id)
                 .then( res => {
                     this.id            = res.data.id;
        ...
        addData() {
            ...
            if (is_valid) {
                ...
                instance.post('book/category', params)
                    .then(res => {
                        this.loaded = 1;
                        if (res.data.status) {
        ...
        updateData(id) {
          ...
          if (is_valid) {
              ...
              instance.post('book/category/edit/'+id, params)
                  .then(res => {
                      this.loaded = 1;
                      if (res.data.status) {
                          this.record = {
        ...
        getData(page) {
          ...
          instance.get('book/category/?page='+page)
            .then(res => {
              this.loaded = 1;
              this.lists = res.data;
            })
            ...
            if( this.check.length > 0 ) {
                ...
                .then((willDelete) => {
                  if (willDelete) {
                      ...
                      instance.post('book/category/remove', params)
                          .then(res => {
                              if(res.status) {
                                  this.getData();
