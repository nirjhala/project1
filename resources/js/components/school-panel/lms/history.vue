              ...
              <table class="table display data-table text-nowrap">
                ...
                <tbody>
                  <tr v-for="(list, i) in lists.data" :key="lists.from + i">
                    ...
                    <td>
                      {{ list.user.mobile }}
                      <span v-if="list.user.mobile2"
                        >, {{ list.user.mobile2 }}</span
                      >
                    </td>
                    ...
                    <td>
                      {{ list.submission_date ? list.submission_date : "-" }}
                    </td>
                  </tr>
                </tbody>
              </table>
...
export default {
  ...
  methods: {
    ...
    searchData(page) {
      ...
      instance
        .get("lms/book/search-issue-history/?page=" + page + "&s=" + s)
        .then((res) => {
          this.loaded = 1;
          if (res.status) {
            this.lists = res.data;
    ...
    getRecords(page) {
      ...
      instance
        .get("lms/book/issue-history/?page=" + page)
        .then((res) => {
          this.loaded = 1;
          if (res.status) {
            this.lists = res.data;
