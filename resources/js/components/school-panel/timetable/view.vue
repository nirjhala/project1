    ...
    <form method="post" @submit.prevent=" deleteRecord() ">
        <div class="card height-auto" v-if="section">
            <div class="card-body">
                ...
                <div v-if="loaded">
                    <div id="timetable">
                        <button type="button" class="btn btn-dark float-right" style="@media print { display: none; }" v-if="!editFlag" v-on:click="editAction(true)"><i class="icon-pencil"></i> Edit</button>
                        <button type="button" class="btn btn-dark float-right" style="@media print { display: none; }" v-if="editFlag" v-on:click="editAction(false)"><i class="icon-times"></i> Close</button>
                        <h4>Time-Table for Class {{ class_full_name }}</h4>
                        <div class="table-responsive" v-if="lists">
                            <table class="table display table-bordered text-nowrap" style="font-size: 10px;">
                                <thead>
                                    <tr>
                                    ...
                                    <tr v-for="(x, i) in lists" :key="i">
                                        ...
                                        <td class="text-center" v-for="(w, i) in weekdays" :key="i">
                                            ...
                                            <div v-if="editFlag == true">
                                                <div class="mb-1">
                                                    <select v-model="subject[x.timeslot_id][w]" class="form-control">
                                                        <option value="">Select Subject</option>
                                                        <option :value="s.id" v-for="(s, i) in subject_lists" :key="i">{{ s.name }}</option>
                                                    </select>
                                                </div>
                                                <div>
                                                    <select v-model="teacher[x.timeslot_id][w]" class="form-control">
                                                        <option value="">Select Teacher</option>
                                                        <option :value="t.id" v-for="(t, i) in teacher_lists" :key="i">{{ t.name }}</option>
                                                    </select>
                                                </div>
                                            </div>
                                        </td>
                    ...
                    <div class="text-center">
                        <button type="button" class="btn-fill-lg btn-gradient-yellow btn-hover-bluedark" v-if="!editFlag" v-on:click="printTimeTable()"> Print </button>
                        <button type="button" class="btn-fill-lg btn-gradient-yellow btn-hover-bluedark" v-if="editFlag" v-on:click="updateData()">Save &amp; Update</button>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        updateData() {
            ...
            instance.post('timetable/edit', params)
                .then(res => {
