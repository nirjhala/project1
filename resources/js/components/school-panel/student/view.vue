    ...
    <div class="dashboard-content-one">
        ...
        <div class="breadcrumbs-area">
            <div class="float-right">
                <router-link :to="{ name: 'AddStudent' }" class="fw-btn-fill btn-gradient-yellow text-white">Add New</router-link>
            </div>
            <h3>Student</h3>
            <ul>
                <li>
            ...
            <div class="row gutters-8 mb-2">
                ...
                <div class="col-lg-2 col-12 form-group">
                    <select v-model.trim="search.department_id" class="form-control">
                        ...
                        <option :value="id" v-for="(name, id) in departments" :key="id">{{ name }}</option>
                    </select>
                </div>
                <div class="col-lg-2 col-12 form-group">
                    <select v-model.trim="search.class_id" class="form-control">
                        <option value="">Select Class</option>
                        <option :value="id" v-for="(name, id) in classes" :key="id">{{ name }}</option>
                    </select>
                </div>
                <div class="col-lg-2 col-12 form-group">
                    <select v-model.trim="search.section_id" class="form-control">
                        <option value="">Select Section</option>
                        <option :value="id" v-for="(name, id) in sections" :key="id">{{ name }}</option>
                    </select>
                </div>
                <div class="col-lg-2 col-12 form-group">
            ...
            <div class="row gutters-8">
                ...
                <div class="col-lg-2 col-12 form-group">
                    <select v-model="search.religion_id" class="form-control">
                        ...
                        <option :value="id" v-for="(name, id) in religions" :key="id">{{ name }}</option>
                    </select>
                </div>
                <div class="col-lg-2 col-12 form-group">
                    <input type="text" v-model="search.cast" placeholder="By Cast" class="form-control">
                </div>
                <div class="col-lg-2 col-12 form-group">
                            ...
                            <table class="table display data-table text-nowrap">
                                ...
                                <tbody>
                                    <tr v-for="(list, i) in lists.data" :key="i">
                                        ...
                                        <td>
                                            <router-link :to="{ name: 'EditStudent', params: {id: list.id} }" data-toggle="tooltip" title="Edit"><i class="fa fa-edit"></i> {{ list.name }}</router-link>
                                        </td>
                                        <td>
                                            <div v-if="list.media">
                                        ...
                                        <td>{{ list.pin_code }}</td>
                                        <td>
                                            <div v-if="list.student_data.section_name && list.student_data.section_name.cls">
                                        ...
                                        <td>
                                            <router-link :to="{ name: 'StudentDetails', params: { id: list.id } }"><i class="icon-eye"></i> Details</router-link>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
...
import { fetchDepartments, fetchClasses, fetchSections, fetchStates, fetchCities, fetchReligions } from "../../../api/script"
...
export default {
    ...
    methods: {
        ...
        searchData(page) {
            ...
            instance.get('search-student/?page='+page+'&'+searchQuery)
                .then(res => {
                    this.loaded = 1;
        ...
        getRecords(page) {
            ...
            instance.get('view-student/?page='+page)
                .then(res => {
                    if(res.data.status) {
                        this.lists = res.data.data;
                    } else {
            ...
            if( this.check.length > 0 ) {
                ...
                .then((willDelete) => {
                  if (willDelete) {
                      ...
                      instance.post('remove-student', params)
                          .then(res => {
                              if(res.status) {
                                  this.getRecords();
