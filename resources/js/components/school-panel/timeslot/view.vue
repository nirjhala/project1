    ...
    <div class="dashboard-content-one">
        ...
        <div class="breadcrumbs-area">
            <div class="float-right">
                <router-link :to="{ name: 'add-timeslot' }" class="fw-btn-fill btn-gradient-yellow text-white">Add New</router-link>
            </div>
            <h3>Timeslot</h3>
            <ul>
                <li>
                            ...
                            <table class="table display data-table text-nowrap">
                                ...
                                <tbody>
                                    <tr v-for="(list, i) in lists.data" :key="i">
                                        ...
                                        <td>
                                            <router-link :to="{ path: 'edit-timeslot/'+list.id, params: list }" data-toggle="tooltip" title="Edit"><i class="fa fa-edit"></i> {{ list.name }}</router-link>
                                        </td>
...
export default {
    ...
    methods: {
        ...
        searchData(page) {
            ...
            instance.get('search-timeslot/?page='+page+'&s='+s)
                .then(res => {
                    this.loaded = 1;
                    if(res.status) {
        ...
        getRecords(page) {
            ...
            instance.get('view-timeslot/?page='+page)
                .then(res => {
                    this.loaded = 1;
                    if(res.status) {
                        this.lists  = res.data.data;
            ...
            if( this.check.length > 0 ) {
                ...
                .then((willDelete) => {
                  if (willDelete) {
                      ...
                      instance.post('remove-timeslot', params)
                          .then(res => {
                              if(res.status) {
                                  this.getRecords();
