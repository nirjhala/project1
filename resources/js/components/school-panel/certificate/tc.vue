    ...
    <div class="dashboard-content-one">
        ...
        <div class="breadcrumbs-area">
            <div class="float-right">
                <router-link :to="{ name: 'AddTc', params: { role: title } }" class="fw-btn-fill btn-gradient-yellow text-white">Add New</router-link>
            </div>
                                ...
                                <table class="table display data-table text-nowrap">
                                    ...
                                    <tbody>
                                        <tr v-for="(list, i) in lists.data" v-bind:key="i">
                                            ...
                                            <td>{{ list.class }}</td>
                                            <td>{{ list.section }}</td>
                                            <td>{{ list.issue_date | formatDateOnly  }}</td>
                                            <td>
                                                <a 
                                                    :href="`${baseURL}api/student/tc/${list.tc_number}`"
                                                    target="_blank"
                                                >
...
import api, { multiple_delete_tc, view_tc } from '../../../api/script'
...
export default {
    ...
    methods: {
        ...
        getRecords(page) {
            ...
            this.loaded = 0;
            view_tc(`page=${page}&search=${this.search.s}`)
                .then(res => {
                    this.lists = res.data
                    this.loaded = 1;
                })
