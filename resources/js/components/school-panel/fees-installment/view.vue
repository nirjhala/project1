                ...
                <tbody>
                  <tr v-for="(list, i) in lists.data">
                    ...
                    <td>
                      <router-link
                        :to="{
                          path: 'edit-fees-installment/' + list.id,
                          params: list,
                        }"
                        data-toggle="tooltip"
                        title="Edit"
...
export default {
  ...
  methods: {
    ...
    searchData(page) {
      ...
      instance
        .get("search-fees-installment/?page=" + page + "&s=" + s)
        .then((res) => {
          this.loaded = 1;
          if (res.status) {
    ...
    getRecords(page) {
      ...
      instance
        .get("view-fees-installment/?page=" + page)
        .then((res) => {
          this.loaded = 1;
          if (res.status) {
            this.lists = res.data.data;
      ...
      if (this.check.length > 0) {
        swal({
          ...
          if (willDelete) {
            ...
            instance
              .post("remove-fees-installment", params)
              .then((res) => {
                if (res.status) {
                  this.getRecords();
