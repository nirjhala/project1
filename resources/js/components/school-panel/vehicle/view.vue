...
<div class="dashboard-content-one">
    ...
    <div class="breadcrumbs-area">
        <div class="float-right">
            <router-link :to="{ name: 'add-vehicle' }" class="fw-btn-fill btn-gradient-yellow text-white">Add New</router-link>
        </div>
        <h3>Vehicle</h3>
        <ul>
            <li>
                        ...
                        <table class="table display data-table text-nowrap">
                            ...
                            <tbody>
                                <tr v-for="(list, i) in lists.data" :key="i">
                                    ...
                                    <td>
                                        <router-link :to="{ path: 'edit-vehicle/'+list.id, params: list }" data-toggle="tooltip" title="Edit"><i class="fa fa-edit"></i> {{ list.vehicle_no }}</router-link>
                                        <div>
                                            <small>({{ list.routes_count }} Routes)</small>
                                        </div>
                                    </td>
                                    <td>{{ list.driver_name ? list.driver_name.name : 'N/A' }}</td>
                                    <td>{{ list.type }}</td>
...
export default {
    ...
    methods: {
        ...
        searchData(page) {
            ...
            instance.get('search-vehicle/?page='+page+'&s='+s)
                .then(res => {
                    this.loaded = 1;
                    if(res.status) {
                        this.lists  = res.data;
        ...
        getRecords(page) {
            ...
            instance.get('view-vehicle/?page='+page)
                .then(res => {
                    this.loaded = 1;
                    if(res.status) {
