    ...
    <div class="dashboard-content-one" style="text-transform: capitialize">
        ...
        <div class="breadcrumbs-area">
            ...
            <ul>
                ...
                <li>{{ $route.params.type }} Pay Grade</li>
            </ul>
        </div>
                        ...
                        <div v-if="lists.data && lists.data.length">
                            <div class="heading-layout1">
                                <div class="item-title">
                                    <h3>{{ lists.from }} to {{ lists.to }} of {{ lists.total }} record(s) are showing.</h3>
                                </div>
                                <button 
                                    type="submit" 
                                        ...
                                        <tr v-for="(list, i) in lists.data" v-bind:key="i">
                                            ...
                                            <td>
                                                <router-link
                                                    ...
                                                    title="Edit"
                                                >
                                                    <i class="icon-pencil1"></i> {{ list.name }}
                                                </router-link>
                                            </td>
                                        </tr>
                                    </tbody>
...
import api, { multiple_delete_pay_grade, view_pay_grade } from '../../../api/script'
...
export default {
    ...
    methods: {
        ...
        getRecords (page) {
            ...
            this.loaded = 0
            view_pay_grade (`page=${page}&s=${this.s}`)
                .then(res => {
                    this.lists = res.data
                    this.loaded = 1
                })
            ...
            if( this.check.length > 0 ) {
                ...
                .then((willDelete) => {
                  if (willDelete) {
                      ...
                      multiple_delete_pay_grade (params)
                          .then(res => {
                            this.getRecords()
