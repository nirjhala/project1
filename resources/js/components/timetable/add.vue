                ...
                <form class="new-added-form" @submit.prevent=" id ? updateData() : addData()">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-3 col-sm-6 col-12 form-group">
                            ...
                            <select class="form-control" v-model="section" @change="getSubjects()" required>
                                <option value="">Select Section</option>
                                <optgroup :label="cls.name" v-for="(cls, i) in classes" :key="i">
                                    <option :value="s.id" v-for="(s, i) in cls.sections" :key="i">{{ cls.name }} - {{ s.name }}</option>
                                </optgroup>
                            </select>
                        </div>
                        <div class="col-lg-3 col-sm-6 col-12 form-group">
                            ...
                            <select class="form-control" v-model="shift" @change="getTimeslots()" required>
                                ...
                                <option :value="s.id" v-for="(s, i) in shifts" :key="i">{{ s.shift_name }}</option>
                            </select>
                        </div>
                        <div class="col-lg-3 col-sm-6 col-12 form-group">
                            <label>Weekday *</label>
                            <multiselect v-model="weekday" :options="weekdays" :multiple="true" :close-on-select="false" :clear-on-select="false" :preserve-search="true" placeholder="Select Weekday" label="name" track-by="name" :preselect-first="true" required>
                                <template slot="selection" slot-scope="{ values, isOpen }">
                                    <span class="multiselect__single" v-if="isOpen" style="display: none;"></span>
                    ...
                    <div class="row" v-for="(t, i) in timeslots" :key="i">
                        ...
                        <div class="col-lg-4 col-sm-6 col-12 form-group">
                            ...
                            <select class="form-control" v-model="subject[t.id]">
                                ...
                                <option :value="s.id" v-for="(s, i) in subjects" :key="i">{{ s.name }}</option>
                            </select>
                        </div>
                        <div class="col-lg-4 col-sm-6 col-12 form-group">
                            <label>Teacher</label>
                            <select class="form-control" v-model="teacher[t.id]">
                                ...
                                <option :value="tcr.id" v-for="(tcr, i) in teachers" :key="i">{{ tcr.name }}</option>
                            </select>
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        ...
        getSubjects() {
            ...
            instance.get('subject/' + this.section)
                .then(res => {
