    ...
    <div class="dashboard-content-one">
        ...
        <div class="breadcrumbs-area">
            <div class="float-right">
                <router-link :to="{ name: 'add-section' }" class="fw-btn-fill btn-gradient-yellow text-white">Add New</router-link>
            </div>
            <h3>Section</h3>
            <ul>
                <li>
                            ...
                            <table class="table display data-table text-nowrap">
                                ...
                                <tbody>
                                    <tr v-for="(list, i) in lists.data">
                                        ...
                                        <td>
                                            <router-link :to="{ path: 'edit-section/'+list.id, params: list }" data-toggle="tooltip" title="Edit"><i class="fa fa-edit"></i> {{ list.name }}</router-link>
                                        </td>
                                        <td>{{ list.cls.name }}</td>
                                        <td>{{ list.cls.dept.dept_name }}</td>
                                    </tr>
                                </tbody>
                            </table>
...
export default {
    ...
    methods: {
        ...
        searchData(page) {
            ...
            instance.get('search-section/?page='+page+'&s='+s)
                .then(res => {
                    this.loaded = 1;
                    if(res.status) {
        ...
        getRecords(page) {
            ...
            instance.get('view-section/?page='+page)
                .then(res => {
                    this.loaded = 1;
                    if(res.status) {
                        this.lists  = res.data.data;
            ...
            if( this.check.length > 0 ) {
                ...
                .then((willDelete) => {
                  if (willDelete) {
                      ...
                      instance.post('remove-section', params)
                          .then(res => {
                              if(res.status) {
                                  this.getRecords();
