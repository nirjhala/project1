...
<div class="dashboard-content-one">
    ...
    <div class="breadcrumbs-area">
        <div class="float-right">
            <router-link :to="{ name: 'add-shift' }" class="fw-btn-fill btn-gradient-yellow text-white">Add New</router-link>
        </div>
        <h3>Shifts</h3>
        <ul>
            <li>
                            ...
                            <table class="table display data-table text-nowrap">
                                ...
                                <tbody>
                                    <tr v-for="(list, i) in lists.data">
                                        ...
                                        <td>
                                            <router-link :to="{ path: 'edit-shift/'+list.id, params: list }" data-toggle="tooltip" title="Edit"><i class="fa fa-edit"></i> {{ list.shift_name }}</router-link>
                                            <div>
...
export default {
    ...
    methods: {
        ...
        searchData(page) {
            ...
            instance.get('search-shift/?page='+page+'&s='+s)
                .then(res => {
                    this.loaded = 1;
                    if(res.status) {
        ...
        getRecords(page) {
            ...
            instance.get('view-shift/?page='+page)
                .then(res => {
                    this.loaded = 1;
                    if(res.status) {
                        this.lists  = res.data.data;
            ...
            if( this.check.length > 0 ) {
                ...
                .then((willDelete) => {
                  if (willDelete) {
                      ...
                      instance.post('remove-shift', params)
                          .then(res => {
                              if(res.status) {
                                  this.getRecords();
