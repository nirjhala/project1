...
<div class="dashboard-content-one">
    ...
    <div class="breadcrumbs-area">
        <div class="float-right">
            <router-link :to="{ name: 'add-religion' }" class="fw-btn-fill btn-gradient-yellow text-white">Add New</router-link>
        </div>
        <h3>Religion</h3>
        <ul>
            <li>
                        ...
                        <table class="table display data-table text-nowrap">
                            ...
                            <tbody>
                                <tr v-for="(list, i) in lists.data">
                                    <td>
                                        <div class="form-check" v-if="list.students_count == 0">
                                    ...
                                    <td>
                                        <router-link :to="{ path: 'edit-religion/'+list.id, params: list }" data-toggle="tooltip" title="Edit"><i class="fa fa-edit"></i> {{ list.name }}</router-link>
                                        <div><small>({{ list.students_count }} students)</small></div>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
...
export default {
    ...
    methods: {
        ...
        searchData(page) {
            ...
            instance.get('search-religion/?page='+page+'&s='+s)
                .then(res => {
                    this.loaded = 1;
                    if(res.status) {
        ...
        getRecords(page) {
            ...
            instance.get('view-religion/?page='+page)
                .then(res => {
                    this.loaded = 1;
                    if(res.status) {
                        this.lists  = res.data.data;
            ...
            if( this.check.length > 0 ) {
                ...
                .then((willDelete) => {
                  if (willDelete) {
                      ...
                      instance.post('remove-religion', params)
                          .then(res => {
                              if(res.status) {
                                  this.getRecords();
